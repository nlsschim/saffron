...
class ImageData:
    """
    Dataclass to track images of interest for microglia analysis.
    """
    data: np.ndarray
    file_path: str
    shape: Tuple[int, ...]
...
def validate_im(image: np.ndarray, file_path: str) -> bool:
    ...
    if len(image.shape) < 2:
        logger.error(f"Image must be at least 2D, got shape {image.shape}: {file_path}")
        return False
    
    if len(image.shape) > 4:
        logger.error(f"Image has too many dimensions {image.shape}: {file_path}")
        return False
    ...
    if any(dim < min_size or dim > max_size for dim in image.shape[:2]):
        logger.error(f"Image dimensions out of reasonable range {image.shape}: {file_path}")
        return False
    
    # Check for valid data types
    valid_dtypes = [np.uint8, np.uint16, np.float32, np.float64]
    if image.dtype not in valid_dtypes:
        logger.warning(f"Unusual dtype {image.dtype}, converting to float32: {file_path}")
    ...
    logger.info(f"Image validation passed: {file_path} - Shape: {image.shape}, Dtype: {image.dtype}")
    return True

def load_tif(file_path: Union[str, Path]) -> ImageData:
    """
    Function to load in tif files to be used for ML.
    Part of loading includes using the validate_im function to ensure the image can be used.
...
def load_images_from_directory(directory: Union[str, Path], 
                             file_extensions: list = ['.tif', '.tiff', '.npy']) -> list:
    """
